...
Ma kuandu bu ten un **widget** ki pode apareci en lugaris diferenti — un karta ki ta apareci na sidebar pikenu **ou** na main wide — viewport queries falha. Karta ka ta sabi si el ten 300px di largura no sidebar o 800px no main. Solusan: **container queries**.
...
<div class="card">
  <img src="avatar.jpg">
  <h3>Sara</h3>
  <p>Hospedeira di Brava</p>
</div>
```

- Na **sidebar pikenu (200px)**: avatar i nome **en koluna** (vertikal). Bio skondidu.
- Na **main wide (800px)**: avatar **na skerda**, nome i bio na direita. Tudu vizivel.

Ku **media queries**, bu ta uza largura di **viewport**. Ma viewport pode ser 1500px ku karta na sidebar di 200px! Media queries da resposta **errada**. Container queries da resposta **direita**.
...
<AnnotatedCode
  ...
  notes={[
    ...
    { m: 2, title: "Varianti di kontainer", body: "`flex-col` é o default (kontainer pikenu). `@md:flex-row` ta vira linha só kuandu o **kontainer** txiga 28rem — ka o viewport." },
    { m: 3, title: "Mostra kondisional", body: "`hidden` ta skonde o bio; `@md:block` ta mostra-l só kuandu o kontainer ten spasu (≥ 28rem)." },
  ]}
/>

Si kel `<aside>` é na sidebar (`w-64` = 16rem = 256px), ninhun `@md:` aktiva. Si bu poi el na main (~800px), `@md:` ta aktiva.
...
<section class="grid grid-cols-1 lg:grid-cols-3 gap-4">
  ...
  <aside class="@container">
    <div class="flex flex-col @md:flex-row gap-4 p-4 bg-white rounded-lg shadow-sm">
      <img src="sara.jpg" class="size-16 @md:size-20 rounded-full">
      <div>
        <h3 class="font-bold">Sara</h3>
        <p class="text-sm text-slate-600">Vila Nova Sintra · Brava</p>
...
<div class="@container">
  <div class="@max-md:bg-amber-100">
    Fundu amber só kuandu kontainer é pikenu
  </div>
</div>
```

**`@container-size` — novu na v4.3.** Un `@container` normal ta da só **largura** (inline-size) pa queries. `@container-size` (v4.3) ta kria un *size container* ki tambén ta da **altura** (block-size) — asi bu pode uza unidadis di query di altura: `cqb` (block) i `cqh` (height).
...
:::callout{type=info}
Resort Brava ten un só layout di página (hero + 3 seksan + footer). Container queries ka da valor li. Ma si bu un dia adisiona un **karta di hospedeira reuzavel** ki ta apareci tanto na hero kumo na sidebar di blog, container queries ta resolvi-l di manera elegante. Es lisan é **konseitu**.
:::
...
<KeyTakeaways showHeader={false}>
  ...
  <RezumuItem term="Breakpoints menor">variantis di kontainer son más pikenu ki viewport — es ta funsiona pa widgets, ka tela enteira.</RezumuItem>
