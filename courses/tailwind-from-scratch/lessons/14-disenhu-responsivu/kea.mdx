...
Padran é simples: bu poi un prefixu antes di klasi ki só aplika a un breakpoint pa **riba**.
...
<Flashcard
  ...
  cards={[
    ...
    { term: "lg:", def: "min-width: 1024px — laptop o desktop pikenu. Aplika di 1024px pa riba." },
...
<h1 class="text-2xl md:text-4xl lg:text-6xl">
  Pikenu na mobile, médiu na tablet, grandi na desktop
</h1>
```
...
Es é manera **mobile-first**: bu skrebe stilu mobile primeru (sen prefixu), dipos adisiona overrides pa ekrans más grandi.

:::callout{type=tip}
**Nunka uza `max-*:` kumo padran**. v4 ten el (`max-sm:`, `max-md:`), ma é só pa kazus raru. Razon: mobile-first é más fásil di mante - bu sempri pensa "kuze ta funsiona mínimu, dipos ki ta melhora ku spasu?". Kontráriu é fragili.
:::

<MisconceptionConfront
  belief="O prefixu `sm:` só ta aplika na ekrans **pikenu** — tipu telefoni."
  myth="`sm:` ta signifika 'small', logu nu ta pensa ki el ta aplika só na ekrans pikenu."
  real="`sm:` é un breakpoint `min-width: 640px` — el ta aplika di 640px **pa riba**. Ekrans pikenu (< 640px) ta uza o stilu default sen prefixu."
  proof={[
    '<div class="sm:text-red-500">Olá</div>',
    '/* CSS ki Tailwind ta jera: */',
    '@media (min-width: 640px) { .sm\\:text-red-500 { color: red } }',
    '/* → aplika di 640px pa riba, ka só na telefonis pikenu */',
  ]}
  takeaway="Tudu varianti é `min-width` — el ta liga di kel tamanhu pa riba. Pa kazus só na ekrans pikenu, uza `max-sm:`."
/>
...
Pa bu aplikasan, `sm:` gosi ta ativa na 600px en bez di 640.

### Adisiona un Breakpoint Personalizadu

Si bu presiza di más di 5, é simples:
...
**Klasi `3xl:` i `tv:` ezisti otomatikamenti** kuandu bu defini token. Es é poderozu di v4 — token CSS = klasi disponivel.

<SectionHeading variant="concept" seq={5}>`min-` vs `max-` Variantis</SectionHeading>

Pa kazus raru ki só faze sentidu na ekrans **pikenu**:
...
<KeyTakeaways showHeader={false}>
  ...
  <RezumuItem variant="warning" term="Ka konfundi sm:">`sm:` ka é só pa telefoni — el ta liga di 640px pa riba. Pa ekrans pikenu, uza `max-sm:`.</RezumuItem>
