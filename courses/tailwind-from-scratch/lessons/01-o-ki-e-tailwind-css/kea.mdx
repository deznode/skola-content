...
:::callout{type=tip}
Bu ta nota klasis manera `rounded-md` i `shadow-sm`? Es nomis é atualizadu na Tailwind v4. Na versan antigu (v3), kel boton di riba uzaba `rounded` (sen sufiksu). Na v4, kada klasi ten un tamanho klaru: `rounded-xs`, `rounded-sm`, `rounded-md`, `rounded-lg`.
:::
...
Nu ta odja un izemplu konkretu. Imajina ki Djonzinho sta kria un kartan pa anunsiu di kursu. Sen Tailwind, el ta ten ki skrebe HTML i un fixeru di CSS separadu; ku Tailwind, el ta skrebe mes kuza — sen CSS:
...
| Vantajen | Splikasan |
|----------|------------|
| **Sen inventa nomis** | Ka ten ki pensa "kumo mi ta txoma es klasi?" — utilidadis dja ten nomis klaru |
...
Tailwind v4 (lansadu na Janeru 2025, atualmenti na versan v4.3) traze un **transformasan konpletu**. Es kursu ta ensina v4 di zero, sen vísius di v3.

### Motor Oxide — Velosidadi Asustante

Tailwind v4 ten un motor novu, txamadu **Oxide**, programadu na Rust. Rezultadu:

- **5× más rápidu** pa builds di zero
- **100× más rápidu** pa rebuilds inkremental (~5 millisegundu)
- Bu ta salva un fixeru, i CSS dja sta atualizadu antis di bu volta pa browser

Mesmu na un laptop modestu, es velosidadi ta poupa-bu txeu tempu.

### Más ki sta pa ben na es kursu

Alén di velosidadi, v4 ta traze konfigurasan na CSS ku `@theme` (ka más `tailwind.config.js`), container queries, sonbras di textu, máskaras i transformasan 3D — kada un ku se própriu lisan na frenti.

Pa gosi, kuze ki importa é: v4 é más rápidu **i** ten más poder — i nu ta konstrui tudu isu djuntu, un lisan di kada bez.

<SectionHeading variant="concept" seq={5}>Pa kuze ki Tailwind ta servi?</SectionHeading>
