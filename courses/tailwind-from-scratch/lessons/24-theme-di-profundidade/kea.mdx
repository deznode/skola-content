...
Na Lisan 21 nu kria Resort Brava ku tokens di marka — `--color-mar-*`, `--color-sol-*`, `--font-display`. Es funsiona, ma nu konxe só un kantu di `@theme`. Es lisan ta abri janela kompletu: **kada namespace, kumo el ta mapeia pa utilidadis, i kumo bu ta tira ou substitui padran.**

Pensamentu xavi di v4: **`@theme` ka ta konfigura Tailwind — el ta defini variavel CSS ki Tailwind ta uza pa gera utilidadis.** Si bu kompriende es, tudu resta ta bira intuitivu.
...
<CodeCloze
  lang="css"
  title="Kual prefixu ta gera kual utilidadi?"
...
Si bu muda `--spacing`, tudu ta atualiza:
...
Tailwind ta ben ku txeu tokens padran (kores Material/Tailwind, fonti `sans/serif/mono`, etc). Si bu presiza **só** tokens di bu marka, **tira-s.**
...
Es é "tema kompletu personalizadu". Útil pa un sistema di dezenhu fitxadu (ex. un dashboard interno) unde bu ka kre risku di klasi padran aparece.

**Kuandu uza:**

- `--color-*: initial;` — bu marka ten paleta diferenti i bu ka kre kores Tailwind ta mistura ku bu klasis
- `--*: initial;` — sistema dezenhu kompletu personalizadu, sen herda nada

:::callout{type=tip}
Komesa **sen** `initial`. Adisiona só si bu realmenti kre kontrolu absolutu. Tira tudu padran ta forsa-bu defini ká bez utilidadi báziku manualmenti.
:::

<SectionHeading variant="concept" seq={4}>@theme inline — kuandu un value ten var()</SectionHeading>

Kazu komuns na frameworks moderno: bu ten un fonti personalizadu ki `next/font` ta karrega ou outro mekanismu, i el ta mete un variavel CSS na `:root`. Si bu skrebe es value ku `@theme {}` normal, Tailwind ta salva o `var()` literalmenti — ma `var(--font-inter)` ka izisti na momentu di build, só na browser. Rezultadu: utilidadis ki ka funsiona ou klasi kebradu.
...
<AnnotatedCode
  ...
  notes={[
    { m: 1, title: "Zerar padran", body: "`--color-*: initial` i `--font-*: initial` ta **tira** tudu kor i fonti padran di Tailwind. Só o ki bu defini djuntu ta izisti dipos." },
...
Pa odja lista kompletu di tokens ki bu tema ta mostra:
...
Bu ta odja tudu `--color-*`, `--font-*`, etc. ki Tailwind kompila pa CSS. Es é forma más rápida di konfirma **o ki realmenti ta gera**.

:::callout{type=tip}
DevTools di Chrome/Firefox ta mostra es variavel kuandu bu inspesiona `:root` no elementu HTML. Bu pode mesmu muda-s temporariamenti pa testa mudansas sen rebuild.
:::
...
<Flashcard
  ...
  cards={[
    ...
    { term: "Tema kompletu personalizadu di zero", def: "--*: initial ta reseta tudu tema, dipos bu ta defini di novu.", code: "@theme { --*: initial; ... }", lang: "css" },
  ]}
/>
