...
I li ten **dos mudansas grandes di v4** ki bu meste sabe primeru — nun é só evolusan, é mudansa konseptual.
...
Na v3, kuandu bu poi `border` (o `border-2`), kor era **`gray-200`** otomatikamenti. Tudu bordu na web v3 era sutil gray sen ki bu penso nada.
...
<CompareTable
  ...
  rows={[
    ...
    { label: "O ki bu ta odja", kind: "text", vals: ["Sempri un **sinzentu klaru** sutil", "O **kor di textu** di elementu (ta eredita)"] },
    { label: "Sorpreza pusível?", kind: "text", vals: ["Nau — sempri mesmu sinzentu", "Sin — se textu é pretu, bordura é preta"] },
    { label: "O ki bu meste faze", kind: "text", vals: ["Nada", "**Sempri** spesifika `border-{kor}`"] },
  ]}
/>
...
Si bu ka spesifika, prepara pa surprezas. Pa migrasan di kódiku v3 a v4, **kel é di kuza más kumun di iskeci**.
...
Larguras válidas: 0, 1 (default), 2, 4, 8. (`border-3`, `border-5`, etc. ka izisti — uza valor arbitráriu si bu presisa.)
...
:::callout{type=tip}
**Nunka uza `outline-none` sen substitui** ku otru sinal visual di foku. Pesoa ki uza teklas pa navega ta perde pista de unde sta. Si bu removi outline default, adisiona `focus:ring-2 focus:ring-sky-500` o similar.
:::
...
Resort Brava ten gosi **forma**. Kards parsi ki bu pode toka, boton pa "klika", header termina.
