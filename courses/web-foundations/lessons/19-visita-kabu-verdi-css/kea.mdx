...
:::callout{type=tip}
**Kapstone é mas longu — i é normal.** Lisan kombinadu (teach + prátika) ta totaliza 45 minutu na ritmu di profesor, ma **mas studanti solu ta gasta 50–60 minutu**, prinsipalmenti na seksan di testimunhu unde Grid + Flexbox ta kombina. Kel-li é eksatamenti kel tenpu ki ta presisa. Salva freketamenti i inspesiona kada seksan ku DevTools antes di passa pa próximu.
:::
...
**Pamodi `* { box-sizing: border-box }` é primeru?** Callback di L14: sen reset, kada `padding` ta silensiozamenti **adisiona** pa largura, i grid di 5 koluna ta kuebra. Aplika reset un vés, kosta sintidu.
...
:::callout{type=tip}
**`justify-content: space-between` é pattern di header.** Logo skerda, nav direita, ku tudu spasu sobra na meiu — automátiku. Memoriza-l, bu ta repeti es kombinasan en kada projetu.
:::

## Pasu 2 — Ilhas grid (5×2) ku CSS Grid

Aki é unde Grid gana kursu. 10 ilhas, 5 koluna, 2 linha — **un linha di CSS**.

![Grid template: lines numeradu (1 ti N+1, -1 di fin), tracks, cells](../../infographics/11-css-grid-template.jpg)

:::callout{type=tip}
Es kapstone ta uza tres propriedadi nobu — `border-radius`, `box-shadow` i `object-fit` — ki nu ka prende na M1–M4. Es é bu primeru "Google-it" di verdadi (callback L13): abri MDN, prokura kada un, i odja ki ta faze. Skill di developer ka é sabe tudu di kor — é sabe unde prokura.
:::
...
Salva. Refresh. **Es é momentu wow.** 10 ilhas ta snap nun grid limpu, organizadu, profisional.
...
Trez kartas di musika — Grid pamodi kada karta é se própriu sela:
...
Aki é unde tudu kursu konverji. **Grid pa eskema grandi** (3 koluna externa). **Flexbox pa eskema interno di kada karta** (foto, sitasan, atribuisan enpilhadu vertikal).
...
.testimonial cite {
  ...
  color: #1098ad;
  font-weight: bold;
  font-style: normal;       /* sobrepasa italic default di <cite> */
}
```
...
:::callout{type=tip}
**Grid pa grandi, Flexbox pa pikenu.** Sempri si bu sta indeskidu — pinta mapa: bu kre 2D? Grid. 1D? Flexbox. Es regra ta resolvi 90% di dezision di layout.
:::
...
**Fin pas — polimentu vizual.** Abri pajina. Tudu sta balansadu? `gap`, `padding`, `font-size` — tweaka ti pajina ta parese profisional. Tudu studanti ta gasta 5 minutu di polimentu fin, i é ki faze diferensa entri "ta funsiona" i "ta mostra".

**Debug é esperadu (callback L13).** Pa-lo menus un seksan ta dá problema. Abri DevTools, prokura regra ganhador (callback L11 ku specificity), i fika fixu. Ka tenta `!important` — fika ku ferramenta.

## Erus komun pa evita

- **Skise reset universal.** Sen `* { box-sizing: border-box }`, kada `padding` ta kuebra grid di ilhas silensiozamenti. Primeru kuza pa adisiona, primeru kuza pa kontrola si layout sta kuebra.
- **`start`/`end` (Grid) vs `flex-start`/`flex-end` (Flexbox).** Studantes ta troka es na seksan di testimunhu, unde dos sistema ta kombina. Si alinhamentu ta parece eradu, **es é primeru kuza pa kontrola**.
...
- **Skise DevTools.** Abri grid overlay pa kada `.islands-grid`, `.music-row`, `.testimonials-grid`. Konfirma line numbers. Si overlay ka aparece, bu skise `display: grid`.
- **`!important` ta volta.** Si bu sta tenta uza-l pa "fija un stilu xikadu", **para**. Inspesiona ku DevTools, prokura regra ganhador, otimiza selector (callback L11 ku specificity), ka aniquila ku `!important`.
...
<KeyTakeaways showHeader={true} chrome="inline">
  <RezumuItem term="Reset primeru" variant="warning">`* { box-sizing: border-box; margin: 0; padding: 0; }` — sempri primeru regra.</RezumuItem>
  ...
  <RezumuItem term="Grid + Flexbox" variant="gold">Standard profisional: Grid pa eskema externu, Flexbox pa internu.</RezumuItem>
  <RezumuItem term="grid-column: 1 / -1">"Spani tudu largura" sen importa kuantas koluna existi.</RezumuItem>
  <RezumuItem term="place-items: center">Sentra na dos eixu dentru di kada sela di Grid.</RezumuItem>
