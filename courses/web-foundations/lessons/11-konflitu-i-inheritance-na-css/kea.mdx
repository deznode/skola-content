...
objectives:
  ...
  - "Le un vetor di especificidadi `(a, b, c)` i prevê kual di dos regra ki ta ganha"
...
Kuandu dos regras ta apunta pa mêsmu elementu ku deklarasons konflitudu (izenplu, dos `color` diferenti), browser ta uza un **ordi di especificidadi** pa dicidi.
...
`*` ta **aplika** regra diretu pa kada elementu. É util pa propriedadis ki **ka ta inheri** ma ki nu kre na tudu lugar — izenplu, `box-sizing` (nu ta odja-l na Lisan 14).

**Importanti:** uza `*` so pa reset. Pa stilu jeral di testu, uza `body` + inheritance — é mas leve.

## `!important` — un hack ki bu ta arrependi

Si bu adisiona `!important` na fin di un valor, regra ta ganha tudu — **ignora skala di especificidadi**.
...
### Pasu 4 — testa `!important` un bez i remove
...
- **Skise ki ordi di fonti ta kuebra impati.** Dos regras ku especificidadi igual → kel ki ta ben **mas tarde** ta ganha. Reordena regras pode silensiozamenti muda lojika.
