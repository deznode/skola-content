...
Abri `cesaria.html`. El ta parse un dokumentu di Word di anu 90, sertu? Tudu pretu na branku, letru sen grasa, tudu grudadu. Oji nu ta prende visti nos HTML ku stilu. Ben-vindu a **CSS — Cascading Style Sheets**. CSS ta da kor, font, spasing i layout. Si HTML é osu, CSS é peli.

## Kuze ki é CSS

**CSS** (Cascading Style Sheets) é un linguajen separadu di HTML ki ta diskreve **apariénsia** di pajina. HTML ta fra "kel-li é un titulu, kel-li é un paragrafu"; CSS ta fra "titulu é azul, paragrafu ten 16px di font, fundu é beji".
...
- `h1` — **selector**: ta fra "tudu elementu `<h1>` na pajina".
...
:::callout{type=tip}
Nunka skise `;` na fin di kada deklarasan. Si bu skise, browser ka ta da erru — el ta **silensiozamenti ignora** próximu regra. É un di bugs mas frustranti pa novatu. Hábitu: skrebe sempri `;`, mêsmu na última deklarasan di un bloku.
:::
...
- **Kuandu uza:** **99% di tempu.** É norma na mundu real.

:::callout{type=tip}
`<link rel="stylesheet">` é un tag **void** (sen tag di féxa, kumo `<img>`). El ta mora dentru di `<head>`, nunka dentru di `<body>`. Atributu `rel="stylesheet"` ta fra pa browser "kel ficheru li é folha di stilu", i `href` ta fra unde ta otxa-l.
:::
...
Na VS Code, na mêsmu pasta unde sta `cesaria.html`, klika **File → New File**. Salva ku nomi **`style.css`** (`Cmd+S` / `Ctrl+S`). Ficheru ta sta vaziu pa gosi.
...
- **Skise atributu `href`** na `<link>` → silensiozamenti ka ta karega nada.
...
- **Stilu na regra CSS sen `;` na fin di deklarasan.** Browser ta ignora próximu regra.
...
<KeyTakeaways showHeader={true} chrome="inline">
  ...
  <RezumuItem term="Trez manera">inline (evita), internal (`<style>` na `<head>`), external (`<link>` + `.css` — prinsipal).</RezumuItem>
  <RezumuItem term="link stylesheet" variant="tip">`<link rel="stylesheet" href="style.css" />` é tag void ki ta mora na `<head>`.</RezumuItem>
