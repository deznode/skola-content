...
**Area di fil (fill area)** = `content + padding + border`. É unde `background-color` ta pinta. `margin` é **fora** di fill — sempri transparenti.

:::callout{type=tip}
**Box model default ta adisiona padding i border na width.** Si bu defini `width: 300px` i `padding: 40px`, kaixa ta okupa **380px** (300 + 40 + 40), nau 300. Es ka é intuitivu — nu ta resolvi-l na Lisan 14 ku `box-sizing: border-box`.
:::
...
Pa lembra ordi di kuatru valor: kumesa na **topu** i sigi na sentidu di **relógiu** (top → right → bottom → left).
...
**Importanti:** `width` i `height` ta aplika só na elementu **bloku** (`<div>`, `<p>`, `<h1>`, `<section>`). Pa elementu **inline** (`<span>`, `<a>`, `<em>`), browser ta ignora `width` i `height`. Nu ta ben falar mas di display na Módulu 4.
...
**Kumo ta funsiona:** `auto` ta fra pa browser "kalkula margin igual na dos ladu", ki ta resulta na sentrajen. Pa funsiona, **elementu ten ki ten un `width` definidu** (ou `max-width`). Sen width, kaixa dja é tudu largura disponivel — ka ta sentra nada.
...
### Pasu 3 — testa kolapsu di margin na DevTools
...
- **Box model default ta adisiona padding na width.** `width: 300px` + `padding: 40px` = 380px na pajina. Vai stragar layout. Solusan ta ben na Lisan 14: `box-sizing: border-box`.
...
- **`margin: 0 auto` sen width** ka ta faze nada vizível. Bloku elementu default dja okupa tudu largura.
Browser ta ignora **margin vertikal na inline elementus** (`<span>`, `<a>` sen `display:block`). Padding vertikal ta pinta fundu, ma ka ta inpurra otus linha (ta da overlap).
- **Skise ki margin é transparenti.** `background-color` ka ta apareci na margin — só na content + padding + border (fill area).
...
<KeyTakeaways showHeader={true} chrome="inline">
  ...
  <RezumuItem term="Fill area">`content + padding + border` é unde `background-color` ta apareci. Margin é fora, sempri transparenti.</RezumuItem>
  ...
  <RezumuItem term="Dimensan">`width` i `height` ta aplika na elementus bloku; inline elementus ta ignora-l.</RezumuItem>
  ...
  <RezumuItem term="Pendenti" variant="warning">Box-sizing default ta soma padding i border na width — Lisan 14 ta resolvi-l ku `box-sizing: border-box`.</RezumuItem>
</KeyTakeaways>
