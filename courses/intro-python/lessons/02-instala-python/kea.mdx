...
Antis di kumesa programá, nu meste prepara nôs ferramentas. É manera un kuzinheru ki ta organiza ingredientis antis di kumesa fazi katchupa — bu meste tudu na lugar sertu! Na es lisan, nu ta instala Python, konfigura VS Code, kria un virtual environment, i kore nôs priméru programa.
...
Si bu skise marka "Add python.exe to PATH", Python ka ta funsiona na terminal. Si bu instala sem marka, kore installer otu bez i skodje "Modify" → marka "Add to PATH".
:::
...
:::callout{type=warning}
**(Windows PowerShell):** Si bo odja erru di "execution policy", roda es komandu primeiru:
```powershell
...
Gosi si! Nôs ta skrebe nôs priméru programa Python.
...
**Parabéns! Bo dja skrebe i roda bo priméru programa Python!** 🎉
...
Un **pakoti** é un biblioteka di kódiku ki otru pesoa skrebe pa bo uza. `pip` é programa ki ta instala-l dentru di bo `.venv`.
