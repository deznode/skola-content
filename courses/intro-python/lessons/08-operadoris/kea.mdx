...
Divisan ku `/` SENPRI ta retorna un float (nùmeru ku pontu desimal), mesmu kuandu rezultadu é "interu". `10 / 5` ta da `2.0`, ka `2`. Es é un armadidja komun!
:::
...
<KeyTakeaways showHeader={false}>
  ...
  <RezumuItem term="Presedensia">`**` > `not` > `* / // %` > `+ -` > komparasan > `and` > `or`.</RezumuItem>
