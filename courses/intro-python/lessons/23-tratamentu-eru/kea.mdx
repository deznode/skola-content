...
try:
    nota = float(tekstu)
    # Si es linha ta da eru, bo ka sabi si é di float() o di otru koza
    guarda_na_bazi(nota)
except ValueError:
...
Pa projetus grandi, bu pode kria bo própriu klasis di exsesan. Es ta faze kódiku más klaru i fásil di manté:
...
1. **Izersisiu 1:** Skrebe un funsan `dividi_seguru(a, b)` ki ta retorna rezultadu di `a / b`. Si `b` é zero, retorna mensajen "Ka pode dividi pa zero". Si `a` o `b` ka é numeru, retorna "Argumentus inválidus". Uza try/except.

2. **Izersisiu 2:** Kria un programa ki ta pidi uzuáriu pa mete si idadi (ku `input()`). Si uzuáriu mete algu ki ka é numeru, programa ta pidi di novu (uza un loop `while` ku try/except). Só para kuandu uzuáriu mete un numeru válidu entri 1 i 120.

3. **Izersisiu 3:** Kria un exsesan personalizadu `IdadiInváliduError` i un funsan `valida_idadi(idadi)` ki ta lansa el si idadi é negativu o maior ki 150. Testa ku try/except.

4. **Izersisiu 4:** Skrebe un programa ki ta ler un ficheru di notas (un numeru pa linha), kaptura `FileNotFoundError` si ficheru ka ezisti, kaptura `ValueError` pa linhas ki ka é numeru, i na final (`else`) mostra média das notas válidus.
...
- **`try/except`** — tenta kódiku i kaptura eru si akontesi
- **Exsesons espesífiku primeru** — `ValueError`, `TypeError`, etc. antis di `Exception`
- **`else`** — só roda si `try` ka teve nenhun eru (mantene kódiku separadu)
- **`finally`** — SENPRI roda (limpeza: fexá ficheru, fexá koneksan)
- **`raise`** — lansa eru di bo própriu kódiku pa sinaliza problemus
- **Exsesons personalizadu** — `class MeuError(Exception)` pa erus espesífiku di bo projetu
- **`as e`** — asesa mensajen i atributus di exsesan
- **Regra di oru:** kaptura espesífiku, ka kaptura tudu ku `except:` sen tipu

---

**Prósimu lisan:** [Annotasons di Tipu →](/courses/intro-python/lessons/annotasoens-tipu)
