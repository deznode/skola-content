...
Imajina un artista di trapézi na sirku, ta salta di un baransa pa otu, txeu altu di txon. Normalmenti el ta panha baransa dretu — ma bez en kuandu el pode skorega. Sen un **rede di sigurãnsa** pa baxu, kualker skorega ta un dizastri.
...
Senpri kaptura exsesons espesífiku — ka uza `except:` sozinhu (sen tipu). Es ta ajuda bu identifika exatamenti kuze ki kore mal:
...
### Múltiplu Handlers ku Asesu pa Mensajen
...
`finally` ta roda **senpri** — si teve eru ou si ka teve. É perfeitu pa limpeza: fexá fixeru, fexá koneksan di bazi di dadus, libera rekursu.

```python
def ler_konfigurasan(kaminhu):
    """Ler fixeru di konfigurasan ku garantia di fexamentu"""
    ficheru = None
    try:
        ...
        if ficheru and not ficheru.closed:
            ficheru.close()
            print("Fixeru fexadu ku susesu.")
```

```python
# Midjór: uza with (ki ta faze mesmu koza!)
...
:::callout{type=tip}
`with` ta substitui padran `try/finally/close` pa fixeru. Ma `finally` inda é útil pa otru tipo di limpeza!
:::

<SectionHeading variant="concept" seq={5}>Fluxu Kompletu: try/except/else/finally</SectionHeading>
...
- Bu kre informá kódiku ki ta txoma bu funsan ki algu kore mal
...
<SectionHeading variant="concept" seq={8}>Padran Prátiku: Fixeru ku Exsesons</SectionHeading>

Un izemplu ki kombina tudu ki nos prende — leitura di fixeru ku tratamentu kompletu:

```python
import os

def prosesa_vendas(kaminhu):
    """Ler fixeru di vendas i kalkula total"""
...
<KeyTakeaways showHeader={false}>
  <RezumuItem variant="gold" term="Regra di oru">Kaptura exseson **espesífiku** (`ValueError`, `ZeroDivisionError`…), ka un `except:` solu — asin bu sabe exatamenti kuze ki kore mal.</RezumuItem>
  ...
  <RezumuItem term="else / finally">`else` só ta kore si `try` ka teve eru; `finally` ta kore **senpri** (limpeza: fexa fixeru, libera rekursu).</RezumuItem>
  ...
  <RezumuItem variant="warning" term="Errus kumuns">Ka po tudu kódiku dentu di `try` — só kuze ki **pode** faya; restu na `else`, senan bu ta maská di unde eru ben.</RezumuItem>
</KeyTakeaways>
