...
objectives:
  ...
  - "Skodje tipu di dadus sertu pa kada kazu"
...
Na mundu real, nu ta lida ku diferenti tipus di informasan: númerus, textus, sin/nau. Python tanbe tene diferenti **data types** pa representa kada tipu di informasan.
...
<SectionHeading variant="concept" seq={1}>4 Tipus Fundamentais</SectionHeading>

![Sistema di tipus di Python: int, float, str, bool](../../infographics/02-tipu-di-dadus-python.jpg)

### Integer (`int`) — Númerus Interus

Númerus sen puntu desimal: positivu, negativu ou zeru.
...
Textu dentu di aspas (simples ou dublu).
...
Bu pode uza aspas simples (`'`) ou dublu (`"`) — rezultadu é mesmu:
...
# Útil pa kuandu textu tene aspas dentu
...
### Boolean (`bool`) — Verdaderu ou Falsu

Só tene 2 valores: `True` ou `False` (ku T i F maiúskulu!).
...
<SectionHeading variant="concept" seq={2}>Kuandu Uza Kada Tipu</SectionHeading>
...
print(f"Bolsista: {'Sin' if e_bolsista else 'Nau'}")
print(f"Ilha: {ilha}")
print(f"Kréditus: {kreditusECV} ECV")
```

<SectionHeading variant="concept" seq={3}>TypeError: Kuandu Tipus Ka Ta Kombina</SectionHeading>

Un di erus más komuns pa prinsipianti é **TypeError** — kuandu bu tenta faze un operasan ku tipus inkompatíveis.
...
<CodeDiff
  ...
  note="Ka pode `+` un `str` ku un `int`. Konverti ku `str(idadi)`, ou — midjór — uza un f-string."
  diff={[
...
:::callout{type=tip}
Kuandu bu enkontra `TypeError`, verifika tipus ku `type()`. Kuazi senpri, soluson é konverti un valor ku `int()`, `float()` ou `str()`.
:::

<SectionHeading level={3} variant="practice">Tenta: Koriji Tipus</SectionHeading>
...
<SectionHeading variant="concept" seq={4}>Tabela Rezumu di Konversan</SectionHeading>
...
<SectionHeading variant="concept" seq={5}>Izemplu Prátiku: Loja di Katxupa</SectionHeading>
...
<TerminalBlock
  ...
  lines={[
    ...
    { type: "out", t: "Spesialidadi di kaza: Sin" },
    { type: "out", t: "Kantu porsan bu kre? 3" },
...
<SectionHeading variant="concept" seq={6}>Verifikasan Rápida di Tipus</SectionHeading>
...
<SectionHeading variant="practice" seq={7}>Repete pa Lembra: Funsons di Konversan</SectionHeading>

Antis di pratika, vira es kartas pa fixa kuze ki kada funsan ta faze. Klika pa odja definison.

<Flashcard
  showHeader={false}
  cards={[
    { term: "int()", def: "Ta konverti un valor pa interu, kortandu (trunkandu) parti desimal." },
    ...
    { term: "bool()", def: "Ta konverti un valor pa True ou False — 0, textu vaziu i None ta da False; tudu otu ta da True." },
    { term: "round()", def: "Ta aredonda un desimal pa interu más prósimu — round(3.9) ta da 4." },
    { term: "isinstance()", def: "Ta verifika si un valor é di un sertu tipu i ta da True ou False — más profisional ki type() == pa kel verifikasan." },
  ]}
/>
...
<KeyTakeaways showHeader={false}>
  <RezumuItem variant="gold" term="Regra di oru">Kuandu bu atxa un `TypeError`, verifika tipus ku `type()` — kuazi senpri soluson é konverti un valor ku `int()`, `float()` ou `str()`.</RezumuItem>
  ...
  <RezumuItem variant="warning" term="Erus kumuns">`TypeError` ta parese kuandu bu ta mistura tipus inkompatíveis (`"Maria" + 25`). I `int(3.9)` ta **trunka** pa `3` — pa aredonda, uza `round()`.</RezumuItem>
