...
Na lisun 3, nôs aprendi a **lé** errus. Agora nôs ta aprendi a **trata** kes — a fazi programa reagí di forma intelijenti kuandu algu ta korri mal, na lugar di simplesmente para i mostra un traceback feiu pa uzuáriu.

Imajina un kaixa eletrónika na Praia ki ta para tudu vez ki algen ta mete numeru eradu. Kel ka é atxeitável! Programa profisionál tene ki reagí: mostra mensajen klaru, pidi di novu, o sigui ku otru alternativa.

<Figure src="../../infographics/13-try-except-fluxu.png" caption="Fluxu di try / except / else / finally na Python" />
...
Agora si uzuáriu mete "abc", na lugar di crash, programa ta mostra mensajen amigável.
...
def dividi(a, b):
    ...
    try:
        ...
        print("Erru: argumеntus tene ki ser numerus!")
        return None
    return rezultadu

print(dividi(10, 2))    # 5.0
print(dividi(10, 0))    # Erru: ka pode dividi pa zero! → None
print(dividi("a", 2))   # Erru: argumеntus tene ki ser numerus! → None
```
...
def transferi_saldу(konta_origem, konta_destinu, montanti):
    ...
    try:
        # Kódiku ki pode faya
        if montanti <= 0:
            raise ValueError("Montanti tene ki ser pozitivu!")
...
`raise` ta dizi a Python: "Es é un problema! Para i aviza kemi ta uza nha funsun."
...
def kalkula_média(notas):
    ...
    if not all(isinstance(n, (int, float)) for n in notas):
        raise TypeError("Tudu elementu tene ki ser numeru!")
...
try:
    ...
    print(f"Problema: {e}")  # Problema: Tudu elementu tene ki ser numeru!
```

### Kuandu Uza `raise`?

Uza `raise` kuandu:
- Argumentu inválidu ta txega na bo funsun
- Kondisun di negósiu ka é satisfeitu (saldу insufisienti, idade inválidu)
- Bo kre informá kódigu ki ta txama bo funsun ki algu korreu mal
...
Un ezemplu ki kombina tudu ki nôs aprendi — leitura di ficheru ku tratamentu konpletu:
...
<Figure src="../../infographics/14-ierarakia-di-exsesun.png" caption="Ierarkia di exsesoens na Python — ValueError, TypeError, i más" />
...
1. **Exersísiu 1:** Skrebi un funsun `dividi_seguru(a, b)` ki ta retorna rezultadu di `a / b`. Si `b` é zero, retorna mensajen "Ka pode dividi pa zero". Si `a` o `b` ka é numeru, retorna "Argumentus inválidus". Uza try/except.
...
3. **Exersísiu 3:** Kria un exsesun personalizadu `IdadiInváliduError` i un funsun `valida_idadi(idadi)` ki ta lansa el si idadi é negativu o maior ki 150. Testa ku try/except.

4. **Exersísiu 4:** Skrebi un programa ki ta lé un ficheru di notas (un numeru por linha), kaptura `FileNotFoundError` si ficheru ka ezisti, kaptura `ValueError` pa linhas ki ka é numeru, e na final (`else`) mostra média das notas válidus.
...
**Prósimu lisun:** [Annotasoens di Tipu] →
