...
Imajina bu ta kaminha i txiga na un **garfu di kaminhu** (fork in the road): un kaminhu ta bai pa direita, otu pa isquerda. Bu tem ki skodje sô un — bu ka pode sigui dos kaminhu na mesmu tempu.

Na programasan, **kondisional** (`if`/`elif`/`else`) funsiona mesmu manera: programa ta txiga na un pontu undi el meste skodje kal kaminhu (bloku di kódiku) pa sigui, baseadu na si un kondisan é verdadi ô falsu. Sô un bloku ta izekutadu — nunka dos na mesmu tempu.

:::callout{type=info}
Un garfu di kaminhu na vida real é normalmenti un eskolha di un bez sô — dipos ki bu skodje direita ô isquerda, ka tem volta. Ma un `if`/`elif`/`else` na kódiku é reavaliadu **kada bez ki el ta izekutadu** — si el sta dentu di un **loop** (bu ta prende loops na Lisan 10), mesmu "garfu" pode manda kódiku pa un kaminhu diferenti na kada pasada, dipendendu di kumo kondisan muda. Metáfora di garfu ka ta kobri kel parti — el ta bon pa kompriende **un desizan sô**, ka pa desizons ripetidu.
:::
