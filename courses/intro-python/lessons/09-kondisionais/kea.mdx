...
:::callout{type=tip}
**Dika:** Nunka skexi dois-pontus `:` dipos di `if` i `else`. Python ta uza indentasan (4 espasu) pa sabi ki kódiku ta pertensi a kál bloku.
:::
...
:::callout{type=tip}
**Dika:** Órdin ta importa! Python ta para na primeru kondisan ki é `True`. Si bo po `idadi < 65` primeru, un kriansa di 8 anu ta kái nkel bloku pamodi `8 < 65` tanbe é `True`.
:::
...
:::callout{type=tip}
**Dika:** Kuidadu ku kondisionais aninhadu demás! Si bu tene más di 3 nivéis, programa ta fika difísil di ler. Más tardi nos ta prende formas di simplifika.
:::
...
:::callout{type=tip}
**Dika:** Nota kumé ki nos po un `if` dentu di `elif operasan == "/"` pa verifika si `num2` é zeru. Divizun pa zeru ta kauza `ZeroDivisionError` na Python!
:::

## Kondisionais na Un Linha (Ternary Operator)

Pa kondisionais simples, Python ta permiti skrebe tudu na un linha só:
...
:::callout{type=tip}
**Dika:** `match/case` é opsional -- `if/elif/else` ta funsiona senpri. Ma `match/case` é más eleganti kuandu bu tene txeu opsan pa kompara. Nos ta uza más di el na Module 2 ku estruturas di dadus.
:::
...
1. **Izersisiu 1: Klassifikador di IMC** -- Pidi uzuáriu pa inseri pezu (kg) i altura (m). Kalkula IMC (pezu / altura ** 2) i mostra klasifikasan:
   - Menor ki 18.5: "Baxu pezu"
   - 18.5 a 24.9: "Pezu normal"
   - 25.0 a 29.9: "Sobrepezu"
   - 30 ó más: "Obezidadi"

2. **Izersisiu 2: Konversor di moeda** -- Kria un programa ki konverti entri Eskudu (ECV), Euro (EUR) i Dólar (USD). Pidi montanti i moeda di orijin, dipos mostra valór na kes otu dos moedas. (1 EUR = 110 ECV, 1 USD = 100 ECV)

3. **Izersisiu 3: Klasifikador di triángulu** -- Pidi 3 ladus di un triángulu. Primeru, verifika si ladus ta forma un triángulu válidu (soma di kualker dos ladus ten ki ser maior ki terseiru). Dipos, klasifika:
...
- `if` ta verifika un kondisan i izekuta bloku si é `True`
- `elif` ta djunta kondisan adisional (pode tene txeu `elif`)
- `else` ta apanha tudu ki ka kombina ku kondisan anterióris
- Kondisionais aninhadu ta permiti lójika más kompleksu, ma kuidadu ku profundidadi
- Operadoris lójiku (`and`, `or`, `not`) ta kombina kondisan
- Ternary operator (`valór if kondisan else otu_valór`) é útil pa kazus simples
- `match/case` (Python 3.10+) é un alternativa modernu pa kadeia di `if/elif`
- Senpri uza `==` pa komparasan, nunka `=`

---

**Prósimu lisan:** [Loops (for/while) →](/courses/intro-python/lessons/loops)
