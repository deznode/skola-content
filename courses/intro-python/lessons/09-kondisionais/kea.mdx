...
Na vida real, nos ta toma desizan a tudu momentu: "Si sta txobe, N ta leba garda-txuva. Si nau, N ta bai sen el." Programas ta funsiona mesmu maneira — és ta presiza toma desizan baseadu na kondisan. Na Python, nos ta uza `if`, `elif` i `else` pa kontrola kaminhu ki programa ta sigi. Dipos di kel lisan li, bu ta sabi faze bu programa pensa i disidi!

Imajina bu ta kaminha i txiga na un **garfu di kaminhu** (fork in the road): un kaminhu ta bai pa direita, otu pa isquerda. Bu tem ki skodje sô un — bu ka pode sigi dos kaminhu na mesmu tempu.

Na programasan, **kondisional** (`if`/`elif`/`else`) funsiona mesmu manera: programa ta txiga na un pontu unde el meste skodje kual kaminhu (bloku di kódiku) pa sigi, baseadu na si un kondisan é verdadi ou falsu. Sô un bloku ta izekutadu — nunka dos na mesmu tempu.

:::callout{type=info}
Un garfu di kaminhu na vida real é normalmenti un eskolha di un bez sô — dipos ki bu skodje direita ou isquerda, ka tem volta. Ma un `if`/`elif`/`else` na kódiku é reavaliadu **kada bez ki el ta izekutadu** — si el sta dentu di un **loop** (bu ta prende loops na Lisan 10), mesmu "garfu" pode manda kódiku pa un kaminhu diferenti na kada pasada, dipendendu di kumo kondisan muda. Metáfora di garfu ka ta kobri kel parti — el ta bon pa kompriende **un desizan sô**, ka pa desizons ripetidu.
:::

<GlossaryText
  text="Un programa ta toma desizan avaliandu un [[kondisan]] — un pergunta ki ta da `True` ou `False` (un valor [[boolean]])."
  terms={{
    kondisan: { en: "condition", definition: "Un expresan ki Python ta avalia kumo `True` ou `False` — pa izemplu idadi >= 18. É kuze ki un if ta verifika antis di disidi." },
...
<SectionHeading variant="concept" seq={1}>If i Else Báziku</SectionHeading>

![Fluxu di kontrol ku if / elif / else](../../infographics/04-kontrol-fluxu-desizan.jpg)

Estrutura `if` ta verifika un kondisan. Si kondisan é `True`, Python ta izekuta bloku di kódiku dentu. Si é `False`, el ta salta pa `else`.
...
<SectionHeading variant="concept" seq={2}>Kadeia If/Elif/Else</SectionHeading>
...
:::callout{type=tip}
Órdin ta importa! Python ta para na primeru kondisan ki é `True`. Si bu po `idadi < 65` primeru, un kriansa di 8 anu ta kái nkel bloku pamodi `8 < 65` tanbe é `True`.
:::
...
<SectionHeading variant="concept" seq={3}>Kondisionais Aninhadu</SectionHeading>

Bu pode po un `if` dentu di otu `if`. Es ta txoma "kondisionais aninhadu" (nested conditionals). Útil kuandu bu presiza verifika más ki un koza di bez.

```python
# Verifika si numeru é par/ímpar i positivu/negativu
...
<SectionHeading variant="concept" seq={4}>Izemplu Prátiku: Verifikador di Anu Bisextu</SectionHeading>
...
<SectionHeading variant="concept" seq={5}>Kalkuladora Simples</SectionHeading>
...
<SectionHeading variant="concept" seq={6}>Kondisionais na Un Linha (Ternary Operator)</SectionHeading>
...
<SectionHeading variant="concept" seq={7}>Kondisionais ku Operadoris Lójiku</SectionHeading>
...
<SectionHeading variant="concept" seq={8}>Pitu di Match/Case (Python 3.10+)</SectionHeading>
...
<SectionHeading variant="concept" seq={9}>Erus Komun ku Kondisionais</SectionHeading>
