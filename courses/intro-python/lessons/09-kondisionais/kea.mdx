...
Na vida real, nôs ta toma desizun a tudu momentu: "Si sta txobê, nha ta léba garda-txuva. Si nau, nha ta bai sin el." Programas ta funsiona mesmu maneira -- és ta presiza toma desizun baseadu na kondisaun. Na Python, nôs ta uza `if`, `elif` e `else` pa kontrola kaminhu ki programa ta sigui. Dipos di kel lisun li, bo ta sabi fazi bo programa pensa e disidi!

<Figure src="../../infographics/04-kontrol-fluxu-desizaun.png" caption="Kontrol di fluxu: fluxograma di if / elif / else na Python" />
...
:::callout{type=tip}
**💡 Dika:** Nunka skexi dois-pontus `:` dipos di `if` e `else`. Python ta uza indentasun (4 espasu) pa sabi ki kódiku ta pertensi a kál bloku.
:::
...
Kuandu bo tene más ki dos opsaun, bo ta uza `elif` (ki é "else if" abreviadu). Python ta verifica kada kondisun di riba pa baxu e ta izekuta priméru ki é `True`.
...
:::callout{type=tip}
**💡 Dika:** Kuidadu ku kondisionais aninhadu demás! Si bo tene más di 3 nivéis, programa ta fika difísil di lé. Más tardi nôs ta aprendi formas di simplifika.
:::
...
Un anu é bisextu si:
- É divisível pa 4 **I**
- **NAU** é divisível pa 100 **OU** é divisível pa 400
...
Bo tanbe podi skrévi mesmu lójika na un linha ku operadoris `and` e `or`:
...
Agora nôs ta djunta tudu ki nôs ta aprendi pa kria un kalkuladora:
...
if operasun == "+":
    ...
    if num2 != 0:
        ...
        print("Erru: Nau podi dividi pa zeru!")
else:
    print(f"Operasun '{operasun}' nau é válidu. Uza +, -, * ó /.")
```
...
Bo podi kombina kondisun ku `and`, `or` e `not`:
...
if temperatura > 25 and not sta_txobê:
    ...
    print("Talves oxi nau é milhor dia.")
```
...
`case _` é komu `else` -- el ta apanha tudu ki nau kombina ku kazus anterióris.
...
match dia:
    ...
    case "sábadu" | "dumingu":
        print("Fin di simana! 🎉")
    case _:
        print("Kel nau é un dia di simana válidu.")
```
...
# if "18" == 18:     # False! string nau é igual a int
if int("18") == 18:  # True ✓
    print("Igual!")
```
...
1. **Exersísiu 1: Klassifikador di IMC** -- Pidi uzuáriu pa inseri pezu (kg) e altura (m). Kalkula IMC (pezu / altura ** 2) e mostra klasifikasun:
   - Menor ki 18.5: "Baxu pezu"
   - 18.5 a 24.9: "Pezu normal"
   - 25.0 a 29.9: "Sobrepezu"
   - 30 ó más: "Obezidadi"

2. **Exersísiu 2: Konversor di moeda** -- Kria un programa ki konverti entre Eskudu (ECV), Euro (EUR) e Dólar (USD). Pidi montanti e moeda di orijin, dipos mostra valór na kes otu dos moedas. (1 EUR = 110 ECV, 1 USD = 100 ECV)

3. **Exersísiu 3: Klasifikador di triángulu** -- Pidi 3 ladus di un triángulu. Priméru, verifica si ladus ta forma un triángulu válidu (soma di kualker dos ladus tene ki ser maior ki terseiru). Dipos, klasifika:
...
- `if` ta verifica un kondisun e izekuta bloku si é `True`
- `elif` ta djunta kondisun adisional (podi tene txeu `elif`)
- `else` ta apanha tudu ki nau kombina ku kondisaun anterióris
