...
Nu dja prende 4 estruturas di dadus fundamentais: listas, tuplas, disionárius, i konjuntus. Gosi é óra di po tudu djuntu! Na kel lisan li, nu ta rezolve problemas real ki bu ta nkontra na dia-a-dia kumo programador. Kada izemplu ta mostra kumé ki bu ta skodje estrutura sertu pa situasan sertu.

![Komparasan di 4 estruturas: lista, tuple, disionáriu, konjuntu](../../infographics/06-4-estruturas-komparasan.jpg)

<SectionHeading variant="concept" seq={1}>Lista di Tarefa (To-Do List)</SectionHeading>
...
tarefas.append("Faze ezersísiu di matemátika")
...
Dipos di kuatru `.append()`, kuze ki `len(tarefas)` ta da? *(Resposta: `4`.)*
:::

```python
# Marka tarefa kumo kompletadu (elimina)
tarefa_feitu = "Kunpra material pa skola"
if tarefa_feitu in tarefas:
    tarefas.remove(tarefa_feitu)
    print(f"\n✅ Kompletadu: '{tarefa_feitu}'")
...
```python
# Versan más kompletu ku prioridadi
...
<SectionHeading variant="concept" seq={2}>Nota di Estudanti (Student Grades)</SectionHeading>
...
```python
# Notas di Maria na diferenti matérias
...
:::callout{type=tip title="Para i pensa"}
Ku `notas_maria = [17, 14, 16, 12, 15]`, kuze ki `sum(notas_maria) / len(notas_maria)` ta da? *(Resposta: `74 / 5 = 14.8`.)*
:::
...
<SectionHeading variant="concept" seq={3}>Inventáriu di Loja (Store Inventory)</SectionHeading>
...
<SectionHeading variant="concept" seq={4}>Removi Duplikadus (Sets + Lists)</SectionHeading>
...
```python
# Senáriu 3: Verifika intersesan -- ki klientis ta kunpra na ambos lojas?
...
<SectionHeading variant="concept" seq={5}>Izemplu Final: Kombina Tudu Estruturas</SectionHeading>

Gosi nu ta uza **listas, tuplas, disionárius i sets** djuntu pa rezolve un problema más kompleksu — un sistema di jestan di evento kultural:
...
<SectionHeading variant="concept" seq={6}>Rezumu: Kuandu Skodje Kual Estrutura</SectionHeading>
...
<KeyTakeaways showHeader={false}>
  ...
  <RezumuItem>Problema real ta presiza kuazi senpri un **kombinasan** di estruturas — uza kada un pa kuze ki el ta faze midjór.</RezumuItem>
