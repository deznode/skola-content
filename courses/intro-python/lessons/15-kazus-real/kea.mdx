...
:::callout{type=tip}
**Dika:** Disionárius ta permiti asesu rápidu pa xavi (nomi di produktu). `.get()` é más siguru ki `[]` pamodi el ta retorna `None` en bez di `KeyError` si produktu ka ezisti.
:::
...
<KeyTakeaways>
  ...
  <RezumuItem variant="tip" term="Pista">`zip()` ta perkore dos lista djuntu (`for materia, nota in zip(...)`), i `collections.Counter` ta konta frekuénsia diretu.</RezumuItem>
</KeyTakeaways>
