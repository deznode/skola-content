...
konteúdu = ficheru.read()
ficheru.close()  # Si skesi es linha, ficheru fika abertu na memória
```
...
`'w+'` dixa bo skrebe i dipos ler mesmu ficheru. Ma tene un armadilha: dipos di skrebe, **kursor ta na final** di ficheru. Bo presiza uza `seek(0)` pa volta pa kumesu!
...
1. **Izersisiu 1:** Kria un programa ki ta skrebe un lista di 5 ilhas di Kabu Verdi nun ficheru txomadu `ilhas.txt` (un ilha pa linha). Después ler ficheru i mostra kada ilha ku si número: `1. Santiago`, `2. São Vicente`, etc.

2. **Izersisiu 2:** Kria un "diáriu simples" — programa ki ta pidi uzuáriu pa skrebe un nota (ku `input()`), adisiona na ficheru `diariu.txt` ku modu `'a'`, i dipos mostra tudu notas ki dja foi skrebidu.

3. **Izersisiu 3:** Skrebe un programa ki ta ler un ficheru di notas (un número pa linha), kalkula média, i skrebe rezultadu nun ficheru novu txomadu `rezultadu.txt`. Uza `os.path.exists()` pa verifika si ficheru di notas ezisti antis di tenta ler.
...
- **Modus prinsipais:** `'r'` (ler), `'w'` (skrebe — apaga!), `'a'` (adisiona), `'w+'` (skrebe + ler)
- **Senpri uza `with`** — ta garanti ki ficheru ta fexá mesmu si eru akontesi
- **Ler:** `read()` (tudu), `readline()` (un linha), `readlines()` (lista), o `for linha in ficheru`
- **Skrebe:** `write(string)` i `writelines(lista)` — ka ta adisiona `\n` automátikamenti
- **`seek(0)`** — presiza dipos di skrebe ku `'w+'` pa volta pa kumesu
- **`os.path`** — kria kaminhus kompatível, verifika si ficheru ezisti
- **Kuidadu ku `'w'`** — ta destrui tudu konteúdu anteriu!

---

**Prósimu lisan:** [Tratamentu di Eru →](/courses/intro-python/lessons/tratamentu-eru)
