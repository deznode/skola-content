...
objectives:
  ...
  - "Trabadja ku kaminhu di ficheru uzandu módulu os.path"
---

# Operasons ku Ficheru

Programas di mundu real presiza guarda dadus — nota di estudantis, lista di klientes, konfigurasan di aplikasan. Tudu es ta vivi na **ficheru** na disku. Na es lisan, nos ta prende kumo ler, skrebe, i adisiona konteúdu na ficherus uzandu Python.
...
<SectionHeading variant="concept" seq={1}>Modus di Ficheru</SectionHeading>

![Modus di ficheru: r, w, a, r+, w+, rb, wb](../../infographics/12-modus-di-ficheru.jpg)

Antis di abri un ficheru, bu presiza dizi pa Python **o ki bu kre faze** ku el. Es é modus prinsipais:
...
<SectionHeading variant="concept" seq={2}>Abri i Ler Ficheru</SectionHeading>
...
<SectionHeading variant="concept" seq={3}>O ki É `with` Statement?</SectionHeading>
...
<SectionHeading variant="concept" seq={4}>Skrebe na Ficheru</SectionHeading>
...
Modu `'w'` é manera "apagador" — el ta limpa tudu i kumesa di novu. Si bu kre mantene o ki dja stá, uza `'a'`!
:::
...
:::callout{type=info}
Es seksan é **opsional** — pa kumesa, trez modus `r`/`w`/`a` di riba ta txega. Volta li kuandu bu presiza ler i skrebe mesmu ficheru na un só operasan.
:::

`'w+'` dixa bu skrebe i dipos ler mesmu ficheru. Ma tene un armadidja: dipos di skrebe, **kursor ta na final** di ficheru. Bu presiza uza `seek(0)` pa volta pa kumesu!
...
<SectionHeading variant="concept" seq={6}>Kaminhu di Ficheru ku `os.path`</SectionHeading>
...
# Verifika si é ficheru ou pasta
...
<SectionHeading variant="concept" seq={8}>Izemplu: Kopia Ficheru ku Múltiplu Kontekstus</SectionHeading>
...
<SectionHeading variant="concept" seq={9}>Erus Kumun ku Ficheru</SectionHeading>
...
<KeyTakeaways showHeader={false}>
  <RezumuItem variant="gold" term="Regra di oru">Senpri uza `with open(...) as f:` — el ta fexa ficheru automátikamenti, mesmu si un eru akontese.</RezumuItem>
  <RezumuItem term="Modus">`"r"` (lé), `"w"` (skrebe — **apaga** tudu!), `"a"` (adisiona na fin), `"w+"` (skrebe + lé).</RezumuItem>
  <RezumuItem term="Lé">`read()` (tudu), `readlines()` (lista), ou `for linha in ficheru` (linha pa linha — midjór pa ficheru grandi).</RezumuItem>
  ...
  <RezumuItem variant="warning" term="Errus kumuns">Modu `"w"` ta **destrui** tudu konteúdu anteriu — uza `"a"` si bu kre mantene-l. I `"r"` nun ficheru ki ka izisti ta da `FileNotFoundError`.</RezumuItem>
</KeyTakeaways>
